<div>
    ...
    <form class="card card-padding card-flex" wire:submit.prevent="submit" wire:init="load" wire:loading.class="opacity-50 pointer-events-none" wire:target="load, submit">
        ...
        <div class="justify-between card-buttons">
            <button class="button button-wide button-gray" type="button" @click="$dispatch('prev-step')">Back</button>
            <button class="button button-wide" type="submit">Next</button>
        </div>
    </form>

    <div class="hidden card-flex" x-data="{
        activeTab: 1,
    }">
        {{-- Rental Name --}}
        <div class="card card-padding">
            ...
            <div class="card-flex" x-show="activeTab == 1" x-collapse :class="activeTab == 1 ? '!overflow-visible' : 'overflow-hidden'">
                <div class="mt-8 transition delay-150 card-form" :class="activeTab == 1 ? 'opacity-100' : 'opacity-0'">
                    <x-forms.text wiremodel="rental_name" label="Rental Name" desc="Give your rental property a name." />
                </div>
                <div class="flex justify-end">
                    <button class="button button-wide" type="button" x-on:click="activeTab++">Next</button>
                </div>
            </div>
        </div>
        ...
        <div class="card card-padding">
            <div class="card-header">
                ...
                <p>Enter the address for the rental property.</p>
            </div>
            <div class="card-flex" x-show="activeTab == 2" x-collapse :class="activeTab == 2 ? '!overflow-visible' : 'overflow-hidden'">
                <div class="mt-8 transition delay-150 card-form" :class="activeTab == 2 ? 'opacity-100' : 'opacity-0'">
                    <x-forms.text class="capitalize" type="text" wiremodel="rental_street" label="Street Address" placeholder="Street Address" />
                    <x-forms.text class="capitalize" type="text" wiremodel="rental_city" label="City" placeholder="City" />
                    <div class="!col-span-8">
                        <x-forms.select wiremodel="rental_state" label="State" :options="\App\Helpers\GeographyHelper::getStates()" placeholder="State" />
                ...
                <div class="flex justify-between transition delay-150" :class="activeTab == 2 ? 'opacity-100' : 'opacity-0'">
                    <button class="button button-gray button-wide" type="button" x-on:click="activeTab--">Back</button>
                    <button class="button button-wide" type="button" x-on:click="activeTab++">Continue</button>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="card-flex" x-ref="div1" x-show="active == 2" x-collapse :class="active == 2 ? '!overflow-visible' : 'overflow-hidden'">
                ...
                <div class="flex items-center justify-between transition delay-150" :class="active == 2 ? 'opacity-100' : ' opacity-0'">
                    ...
                    <div class="flex items-center space-x-2">
                        <x-spinner />
                        <button class="button button-wide button-primary" type="button" wire:loading.attr="disabled" wire:click="submit">
                            Next
                        </button>
                    </div>
                </div>
                    ...
                    <div class="card-form" x-data="photosuploader" x-on:livewire-upload-start="isuploading = true" x-on:livewire-upload-finish="isuploading = false, $wire.dispatch('init-sortable')" x-on:livewire-upload-error="isuploading = false" x-on:livewire-upload-progress="progress = $event.detail.progress">
                        ...
                        @if ($photos)
                            <div class="p-4 text-xs bg-gray-100 rounded-lg">
                                ...
                                <p>You can reorder the photos by clicking and dragging them in the correct order. The first photo is your header photo.</p>
                            </div>

                            <div class="relative grid grid-cols-2 gap-4 select-none" wire:sortable="updatePhotoOrder" wire:sortable.options="{ animation: 150, dragoverBubble: true }" wire:loading.delay.class="opacity-50 pointer-events-none" wire:target="updatePhotoOrder">
                                @foreach ($photos as $photo_key => $photo)
                                    <div class="relative first:col-span-full group" wire:key="photo-{{ $photo_key }}" wire:sortable.item="{{ $photo_key }}">
                                        <div class="block w-full h-full overflow-hidden bg-gray-100 rounded-lg aspect-w-10 aspect-h-7">
                                            <img class="object-cover" src="{{ $photo->temporaryUrl() }}" alt="" wire:sortable.handle>
                                        </div>
                                        <button class="absolute cursor-pointer group-hover:visible invisible p-1 bg-white rounded-full -top-1.5 -right-2" x-on:click="$wire.deletePhoto({{ $photo_key }})">
