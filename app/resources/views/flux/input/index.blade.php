...
<flux:with-field :$attributes :$name>
    <div data-flux-input {{ $attributes->only('class')->class('w-full relative block group/input') }}>
        <?php if (is_string($iconLeading)): ?>
        <div class="pointer-events-none absolute bottom-0 left-0 top-0 z-10 flex items-center justify-center pl-3 text-xs text-zinc-400/75">
        ...
        <input data-flux-control data-flux-group-target type="{{ $type }}" {{-- Leave file inputs unstyled... --}} {{ $attributes->except('class')->class($type === 'file' ? '' : $classes) }} @isset($name) name="{{ $name }}" @endisset @if ($mask) x-mask="{{ $mask }}" @endif @if ($invalid) aria-invalid="true" data-invalid @endif @if (is_numeric($size)) size="{{ $size }}" @endif>

        <?php if ($kbd): ?>
        <div class="pointer-events-none absolute bottom-0 right-0 top-0 flex items-center justify-center pr-4 text-xs text-zinc-400">
            {{ $kbd }}
        </div>
        <?php endif; ?>

        <?php if (is_string($iconTrailing)): ?>
        <div class="pointer-events-none absolute bottom-0 right-0 top-0 flex items-center justify-center pr-3 text-xs text-zinc-400/75">
        ...
        <?php if ($expandable): ?>
        <div class="absolute bottom-0 right-0 top-0 flex items-center justify-center pr-2">
...
<button {{ $attributes->merge(['type' => 'button'])->class([$classes, 'w-full relative flex']) }}>
    <?php if (is_string($iconLeading)): ?>
    <div class="absolute bottom-0 left-0 top-0 z-10 flex items-center justify-center pl-3 text-xs text-zinc-400/75">
        <flux:icon :icon="$iconLeading" variant="mini" />
    </div>
    <?php elseif ($iconLeading): ?>
    <div {{ $iconLeading->attributes->class('z-10 absolute top-0 bottom-0 flex items-center justify-center text-xs text-zinc-400/75 pl-3 left-0') }}>
    ...
    <div class="dark:text-white/40 block flex-1 self-center text-left font-medium text-zinc-400">
        {{ $attributes->get('placeholder') }}
    </div>
    <?php else: ?>
    <div class="dark:text-white flex-1 self-center text-left font-medium text-zinc-800">
        {{ $slot }}
    </div>
    <?php endif; ?>

    <?php if ($kbd): ?>
    <div class="absolute bottom-0 right-0 top-0 flex items-center justify-center pr-4 text-xs text-zinc-400/75">
        {{ $kbd }}
    </div>
    <?php endif; ?>

    <?php if (is_string($iconTrailing)): ?>
    <div class="absolute bottom-0 right-0 top-0 flex items-center justify-center pr-3 text-xs text-zinc-400/75">
        <flux:icon :icon="$iconTrailing" variant="mini" />
    </div>
    <?php elseif  ($iconTrailing): ?>
    <div {{ $iconTrailing->attributes->class('absolute top-0 bottom-0 flex items-center justify-center text-xs text-zinc-400/75 pr-2 right-0') }}>
